...
app.post('/menu', requireAuth, (req,res)=>{ const { id,name,price,cat,active=true } = req.body||{}; if(!id||!name||!price) return res.status(400).send('id/name/price required'); if(MENU.some(m=>m.id===id)) return res.status(409).send('duplicate id'); MENU.push({ id, name, price:Number(price), cat:cat||'', active:!!active, soldout: !!(req.body&&req.body.soldout) }); res.json({ ok:true }); });
app.patch('/menu/:id', requireAuth, (req,res)=>{ const i=MENU.findIndex(m=>m.id===req.params.id); if(i<0) return res.status(404).send('not found'); MENU[i]={ ...MENU[i], ...req.body, soldout: req.body.soldout!==undefined ? !!req.body.soldout : (MENU[i].soldout||false) }; res.json({ ok:true }); });
...
app.get('/events/orders', (req,res)=>{ res.setHeader('Content-Type','text/event-stream'); res.setHeader('Cache-Control','no-cache'); res.setHeader('Connection','keep-alive'); res.flushHeaders?.(); const client={res}; clients.add(client); req.on('close', ()=> clients.delete(client)); });
...
app.get('/orders', (req,res)=>{ const t=(req.query.table||'').trim(); if(t){ return res.json(ORDERS.filter(o=> String(o.tableNo||'')===String(t))); } res.json(ORDERS); });
app.post('/orders', (req,res)=>{ const { tableNo, items, amount, paymentKey, orderId } = req.body||{}; const o={ id:crypto.randomUUID(), orderId: orderId||`ORD-${Date.now()}`, tableNo, items:items||[], amount:Number(amount)||0, paymentKey:paymentKey||'', status:'접수', createdAt:new Date().toISOString(), cleared:false }; ORDERS.push(o); try{ broadcastOrder(o);}catch(_){} res.json({ ok:true, order:o }); });
...
// Refund (Toss cancel if configured) - admin only
app.post('/refund/:id', requireAuth, async (req,res)=>{
  try{
    ...
    if(secret && ord.paymentKey){
      // Real cancel via Toss Payments
      const basic = Buffer.from(secret+':').toString('base64');
      const r = await fetch('https://api.tosspayments.com/v1/payments/'+encodeURIComponent(ord.paymentKey)+'/cancel', {
        method:'POST',
      ...
      if(!r.ok){
        ...
        return res.status(502).json({ ok:false, message:'Toss cancel failed', detail: txt });
      }
    }
    ORDERS[i] = { ...ord, status:'환불', refundedAt:new Date().toISOString() };
    return res.json({ ok:true, order: ORDERS[i] });
  }catch(e){
    console.error(e);
    res.status(500).json({ ok:false, message:'refund error' });
  }
});
